...
robot_simulator.pin_robot.collision_model.addCollisionPair(
    pin.CollisionPair(
        ...
        robot_simulator.pin_robot.collision_model.getGeometryId("support_link_0"),
    )
)
...
robot_simulator.pin_robot.collision_model.addCollisionPair(
    pin.CollisionPair(
        robot_simulator.pin_robot.collision_model.getGeometryId("panda2_link5_sc_4"),
        robot_simulator.pin_robot.collision_model.getGeometryId("support_link_0"),
    )
)
...
OBSTACLE_POSE2 = robot_simulator.pin_robot.collision_model.geometryObjects[
    robot_simulator.pin_robot.collision_model.getGeometryId("obstacle2")
].placement
OBSTACLE_POSE3 = robot_simulator.pin_robot.collision_model.geometryObjects[
    robot_simulator.pin_robot.collision_model.getGeometryId("obstacle3")
].placement
...
for i in range(sim_data["N_sim"]):
    if i % 2000 == 0 and i != 0:
        ...
        for k in range(T):
            ddp.problem.runningModels[k].differential.costs.costs[
                "gripperPoseRM"
            ].cost.residual.reference = TARGET_POSE.translation
        ddp.problem.terminalModel.differential.costs.costs[
            "gripperPose"
        ].cost.residual.reference = TARGET_POSE.translation
