...
def display_box(p_des, size=[0.1, 0.1, 0.1], color=[1., 1., 1., 1.]):
    ...
    than PyBullet WORLD frame if the base placement in the simulator is not (eye(3), zeros(3))
    INPUT: 
        p_des  : desired position of the box in pinocchio.WORLD
    ...
    visualBoxId = p.createVisualShape(shapeType=p.GEOM_BOX,
...
def display_capsule(p_des, length=1.0, radius=0.1, color=[1., 1., 1., 1.]):
    ...
    than PyBullet WORLD frame if the base placement in the simulator is not (eye(3), zeros(3))
    INPUT: 
        p_des  : desired position of the capsule in pinocchio.WORLD
    ...
    capsuleId = p.createMultiBody(baseMass=0.,
                                  baseInertialFramePosition=[0., 0., 0.],
                                  baseVisualShapeIndex=visualCapsuleId,
                                  basePosition=[0., 0., 0.],
                                  useMaximalCoordinates=True)
